...
## Purpose
Implements `init_knowledge_base` (scaffold empty template files including `AGENTS.md`) and `generate_knowledge_base` (read source code, call Claude, write full knowledge base). These are the bootstrapping operations — run once per project.
...
- `generateKnowledgeBase` may overwrite existing knowledge files (it's a full regeneration). It does not touch `AGENTS.md`.
...
## Files
- `src/scaffold/index.ts` — `initKnowledgeBase`, `generateKnowledgeBase`, inline templates including `TEMPLATE_AGENTS_MD`, private `walkDir`
